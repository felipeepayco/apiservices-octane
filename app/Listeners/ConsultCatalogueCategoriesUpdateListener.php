...
class ConsultCatalogueCategoriesUpdateListener extends HelperPago
{
    ...
    public function handle(ConsultCatalogueCategoriesUpdateEvent $event)
    {
        try {
            ...
            $category=CatalogoCategorias::where("id",$categoryId)
                ->where("cliente_id",$clientId)
                ->where("estado",1)
                ->first();
            if(!$category){
                $validate = new Validate();
                $validate->setError(500, "category no found");
                $success         = false;
                $last_action    = 'validation clientId y data of filter';
                $title_response = 'Error';
                $text_response  = 'Some fields are required, please correct the errors and try again';
            ...
            $category->catalogo_id=$catalogueId;
            $category->fecha=new \DateTime("now");
            $category->estado=1;
            $category->save();
             ...
             $newData = [
                ...
                "catalogueId"=>$category->catalogo_id,
                "date" => $category->fecha->format("Y-m-d H:i:s"),
            ];
