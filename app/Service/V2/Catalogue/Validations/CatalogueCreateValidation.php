...
class CatalogueCreateValidation extends HelperPago
{
    ...
    public function validate2(Request $request)
    {
        ...
        try {
            ...
            $id = CommonValidation::validateIsSet($data, 'id', '0', 'int');
            ...
            $companyName = CommonValidation::validateIsSet($data, 'companyName', null, 'string');
            ...
            if ($ownDomain) {
                CommonValidation::validateParamFormat($data, $validate, $ownDomainValue, 'ownDomainValue', self::EMPTY, true);
            ...
            if ($ownDomain) {
                CommonValidation::validateParamFormat($data, $validate, $ownSubDomainValue, 'ownSubDomainValue', self::EMPTY, true);

            }

            $cname = CommonValidation::validateIsSet($data, 'cname', null);
            if ($ownDomain) {
                CommonValidation::validateParamFormat($data, $validate, $cname, 'cname', self::EMPTY, true);
            ...
            $whatsappActive = CommonValidation::validateIsSet($data, 'whatsappActive', false, 'bool');
            $arr_respuesta['whatsappActive'] = $whatsappActive;

            $origin = CommonValidation::validateIsSet($data, 'origin', "epayco", "string");
            CommonValidation::validateParamFormat($data, $validate, $origin, 'origin', self::EMPTY, true);
            ...
            if (!$finish) {
                ...
                foreach ($banners as $key => $value) {
                    ...
                    if($value !="" && (strlen($value) > 200 || $id === "0"))
                    {
                ...
                if (!CommonValidation::validateBase64Image($img) && ($value =="" || (strlen($value) > 200 && $id === "0"))) {
            ...
            $indicativoPais = CommonValidation::validateIsSet($data, 'indicativoPais', "57", "string");
            ...
            CommonValidation::validateParamFormat($arr_respuesta, $validate, $clientId, 'clientId', self::EMPTY);

            CommonValidation::validateParamFormat($arr_respuesta, $validate, $name, 'name', 'range', true, [1, 50]);
            CommonValidation::validateParamFormat($arr_respuesta, $validate, $name, 'name', self::EMPTY, true);

            CommonValidation::validateParamFormat($arr_respuesta, $validate, $id, 'id', self::EMPTY);

            if (isset($origin) && $origin == CM::ORIGIN_EPAYCO && !$finish) {

                CommonValidation::validateParamFormat($arr_respuesta, $validate, $companyName, 'companyName', self::EMPTY);
                ...
                if ($epaycoDeliveryProvider) {
                    CommonValidation::validateParamFormat($arr_respuesta, $validate, $senderType, 'senderType', self::EMPTY, false);
                    CommonValidation::validateParamFormat($arr_respuesta, $validate, $senderType, 'senderType', 'range', false, [1, 20]);

                    CommonValidation::validateParamFormat($arr_respuesta, $validate, $senderDocType, 'senderDocType', self::EMPTY, false);
                    CommonValidation::validateParamFormat($arr_respuesta, $validate, $senderDoc, 'senderDoc', self::EMPTY, false);
                    CommonValidation::validateParamFormat($arr_respuesta, $validate, $senderDoc, 'senderDoc', 'range', false, [1, 20]);

                    CommonValidation::validateParamFormat($arr_respuesta, $validate, $senderPhone, 'senderPhone', self::EMPTY, false);
                    ...
                    CommonValidation::validateParamFormat($arr_respuesta, $validate, $pickupCity, 'pickupCity', self::EMPTY, false);
                    CommonValidation::validateParamFormat($arr_respuesta, $validate, $pickupCity, 'pickupCity', 'range', false, [1, 50]);

                    CommonValidation::validateParamFormat($arr_respuesta, $validate, $pickupDepartament, 'pickupDepartament', self::EMPTY, false);
                    CommonValidation::validateParamFormat($arr_respuesta, $validate, $pickupDepartament, 'pickupDepartament', 'range', false, [1, 50]);

                    CommonValidation::validateParamFormat($arr_respuesta, $validate, $pickupAddress, 'pickupAddress', self::EMPTY, false);
                    CommonValidation::validateParamFormat($arr_respuesta, $validate, $pickupAddress, 'pickupAddress', 'range', false, [1, 150]);

                    CommonValidation::validateParamFormat($arr_respuesta, $validate, $pickupConfigurationId, 'pickupConfigurationId', self::EMPTY, false);
                ...
                if ($epaycoDeliveryProvider && $senderType == "Personal") {
                    CommonValidation::validateParamFormat($arr_respuesta, $validate, $senderFirstname, 'senderFirstname', self::EMPTY, false);
                    ...
                    CommonValidation::validateParamFormat($arr_respuesta, $validate, $senderLastname, 'senderLastname', self::EMPTY, false);
                    ...
                    CommonValidation::validateParamFormat($arr_respuesta, $validate, $senderBusiness, 'senderBusiness', self::EMPTY, false);
            ...
            if (isset($data["ownDomain"])) {
                ...
                if ($data["ownDomain"] === true && !empty($data["ownDomainValue"]) && !empty($data["ownSubDomainValue"]) && !empty($data["cname"])) {

                    $catalogueResult = $this->catalogueRepository->checkDomainAndSubDomain($data["ownDomainValue"], $data["ownSubDomainValue"]);
                    ...
                    if (!$data["cname"] || $data["cname"] === "") {
                        $validate->setError(500, "cname is invalid");
                    }
                }
            }
            ...
            $has_errors = $validateForbiddenWordService->handle(["nombre" => $name, "nombre_empresa" => $companyName], ["endpoint_action" => "crear", "action" => "Tienda"]);
            if (!$has_errors["success"]) {

                $logService = Container::getInstance()->make(\App\Service\V2\MongoLog\Process\MongoLogService::class);

                $logService->handle(["module" => "Tienda", "action" => "crear", "client_id" => $clientId, "word" => $has_errors["word"]]);
