...
class ConfigurationService extends HelperPago
{
    ...
    public function __construct(CatalogueRepository $catalogueRepository, ProductRepository $productRepository, ShoppingCartRepository $shoppingCartRepository)
    {
    ...
    public function process($data)
    {
        try {
            ...
            if (!$client) {
                ...
                $planArray = BblSuscripcion::checkPlanByDate($clientId, 12, [1, 2, 3, 5, 7, 10], false, true);
                $pendingPlan= BblSuscripcion::checkPendingPlan($clientId);

                $plan = !empty($planArray) ? BblSuscripcion::formatResponsePlan($planArray[0]) : (!empty($pendingPlan)?  BblSuscripcion::formatResponsePlan($pendingPlan):false);
                $totalCatalogsActive = $this->getTotalActive($catalogs);
                $totalProductsActive = $this->getTotalActive($totalProducts);
                $totalShoppingCarts = $this->shoppingCartRepository->getByCriteria(["clienteId"=>$clientId]);
                ...
                $data = [
                    ...
                    "totalCatalogs" => count($totalCatalogs),
                    ...
                    "totalShoppingCarts" => $totalShoppingCarts ? count($totalShoppingCarts) : 0,
                    "plan" => $plan,
                    "permittedPlanFree" => $client->prueba_gratis,
                    "invoiceUrl" => "",
    ...
    private function getCatalogueElastic($clientId, $pending, $isTotal = false)
    {
        ...
        $result = $this->catalogueRepository->getCatalogueConfiguration($clientId, $pending, $isTotal);
        return $result;
    }

    private function getProductElastic($clientId, $pendingCatalogueId, $getTotalProduct = false)
    {
        $result = $this->productRepository->getProductsForConfigurations($clientId, $getTotalProduct, $pendingCatalogueId);
        return $result;
    }
    ...
    public function getTotalActive($items)
    {
        ...
        return array_filter((array) $items, function ($item) {
    ...
    private function getTotalCategories($totalCatalogs)
    {
        ...
        foreach ($totalCatalogs as $tempCatalogues) {
            $countCataloguetemp++;
    ...
    private function getFirstPendingCatalogueId($pendingCatalogs, $catalogsB, $findCatalogueId)
    {
        $catalogs = $catalogsB->toArray();
        $catalogueId = 0;
        $nameCatalogue = "";
        ...
        if ($findCatalogueId > 0) {
            $keyInCatalogs = array_search($findCatalogueId, array_column($catalogs, 'id'));
            $pendingCatalogue = $catalogs[$keyInCatalogs];
            $redefineCatalogue = true;
        }
        if (count($catalogs) == 1 && !empty($pendingCatalogs["data"])) {
            $pendingCatalogue = $pendingCatalogs["data"][0];
            $redefineCatalogue = true;
        }

        if ($redefineCatalogue && !empty($pendingCatalogue)) {
            $pendingCatalogue = (object) $pendingCatalogue;
